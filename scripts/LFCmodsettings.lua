...
_G.LFC.modgetpersistentdata(_G.LFC.MOD_SETTINGS.FILENAME, function(_, data)
...
OptionsScreen._BuildMenu = function(self, subscreener, ...)
    subscreener.sub_screens[_G.LFC.MOD_CODE] = self.panel_root:AddChild(LFCSettingsTab(self))
    local menu = old_OptionsScreen_BuildMenu(self, subscreener, ...)

	local lfc_button = subscreener:MenuButton(_G.LFC.MOD_SETTINGS.TAB_NAME, _G.LFC.MOD_CODE, _G.LFC.MOD_SETTINGS.TOOLTIP, self.tooltip)
...
OptionsScreen.InitializeSpinners = function(self, ...)
    for _, w in pairs(self.subscreener.sub_screens[_G.LFC.MOD_CODE].left_column) do
        if w.type == _G.LFC.SETTING_TYPES.SPINNER then
